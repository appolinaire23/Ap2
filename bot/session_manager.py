...
class SessionManager:
    ...
    def _init_database(self):
        """Initialize session database avec configuration automatique"""
        try:
    ...
    async def store_session(self, user_id, phone_number, session_name):
        ...
        try:
            if not self.db_connection:
                logger.warning("Database connection not available, session not stored")
                return
    ...
    async def get_user_sessions(self, user_id):
        ...
        try:
            if not self.db_connection:
                logger.warning("Database connection not available, returning empty sessions")
                return []
    ...
    async def restore_all_sessions(self):
        ...
        try:
            if not self.db_connection:
                logger.warning("Database connection not available, skipping session restoration")
                return
    ...
    async def update_session_activity(self, user_id, phone_number):
        ...
        try:
            if not self.db_connection:
                logger.warning("Database connection not available, session activity not updated")
                return
    ...
    async def cleanup_expired_sessions(self):
        ...
        try:
            if not self.db_connection:
                logger.warning("Database connection not available, skipping session cleanup")
                return
